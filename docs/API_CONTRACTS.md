...
状态：draft
阶段：第一阶段规格盘点
范围：定义前端、Rust 后端、浏览器扩展、本地插件之间的通信协议。本文不包含任何第三方项目源码或文案。
...
- 2026-07-17 Appearance language catalog：新增只读本地 `appearance_get_language_catalog` / `appearance_export_language_catalog` commands、CLI `language-catalog` / `export-language-catalog`、TypeScript wrapper 和 Settings > Appearance Language catalog 控件。响应包含当前 language、9 个本地 language id/label/native label/text direction、shell dictionary key count 和 source and asset review notes；导出写出 `kind=fetchdock.appearance.language_catalog` envelope。该能力不复制外部 translation files/product copy，不包含 extension locale packs、plugin runtime i18n、下载内容或 Cookie/Auth values，也不执行 full UI string extraction、linguistic QA 或手动 locale 验收。

- 2026-07-17 Appearance theme catalog：新增只读本地 `appearance_get_theme_catalog` / `appearance_export_theme_catalog` commands、CLI `theme-catalog` / `export-theme-catalog`、TypeScript wrapper 和 Settings > Appearance Theme catalog 控件。响应包含当前 theme/language、14 个本地 theme id/label/CSS class/category、9 个 language id、density/font-scale bounds 和 source and asset review notes；导出写出 `kind=fetchdock.appearance.theme_catalog` envelope。该能力不复制外部 theme files/assets/screenshots/logos/text，不读取下载内容、Cookie/Auth values，也不执行 visual QA、contrast measurement 或手动主题验收。
...
| dapp_get_release_document_summaryd | d{}d | d{ generated_at; source_root; document_count; missing_count; total_size_bytes; documents[]; capability_map_status_counts[]; acceptance_status_counts[]; review_notes[] }d |
