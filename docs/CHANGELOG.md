...
- Added `scripts/verify-fast.ps1` and `npm run verify:fast` as a fast implementation preflight that chains typecheck, Rust fmt check, browser-extension static verification, release-packaging guard, originality guard, conflict-marker scan, and literal newline escape scan without running frontend tests, Rust tests, clippy, Tauri build, signing, installer smoke, or browser E2E.
- Added a originality boundary static guard through `scripts/verify-originality-boundary.ps1` and `npm run verify:originality`. It scans source/docs/scripts/extension/packaging text outside generated/build directories for hard external project traces, fails on forbidden project/owner/repository matches, and reports review-only boundary wording counts for release cleanup without changing product behavior.
...
- Added a structured release checklist command and Legal-page panel that reports originality, third-party notice, privacy, diagnostics inventory, capability honesty, updater/signing, packaging smoke, and blocked integration gates without running builds or overstating pending work.

- Added a built-in Legal route/page for originality, dependency-license, user-responsibility, privacy, no-telemetry, and release-gate notices, with About and command-palette navigation.
...
- Expanded the About page with runtime, data root, originality stance, privacy stance, legal note, build status, diagnostics export, and Settings navigation.
...
- Added originality function parity, architecture, API contract, and acceptance documentation.
